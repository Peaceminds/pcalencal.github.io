...
<template>
    <div class="calendar">
        ...
        <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}">
            <td v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled}" @click="selectsss(k1,k2,$event)">
...
<script>
import calendarrr from './calendar.js'
export default {
    props: {
        // 多选模式
        multi: {
        ...
        weeks: {
            type: Array,
            default:function(){
                return window.navigator.language.toLowerCase() == "zh-cn"?['日','一', '二', '三', '四', '五', '六',]:['Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun', 'Mon']
            }
        },
        // 自定义月份
        months:{
    ...
    methods: {
        ...
        // 获取农历信息
        getLunarInfo(y,m,d){
            let lunarInfo=calendarrr.solar2lunar(y,m,d)
        ...
        selectsss(k1, k2, e) {
            ...
            if (this.range) {
                if (this.rangeBegin.length == 0 || this.rangeEndTemp != 0) {
                    ...
                    // console.log("选中日期---------",begin,end,day)
                    this.$emit('selectsss',begin,end)
                }
                this.render(this.year, this.month)
            }else if (this.multi) {
                // 如果已经选过则过滤掉
                ...
                this.$emit('selectsss',this.multiDays)
            } else {
                // 取消上次选中
                ...
                this.today = [k1, k2]
                // console.log("选中日期---------", this.today[1])
                this.$emit('selectsss',[this.year,this.zero?this.zeroPad(this.month + 1):this.month + 1,this.zero?this.zeroPad(this.days[k1][k2].day):this.days[k1][k2].day], this.today[1])
            }
        },
        changeYear(){
            if(this.yearsShow){
